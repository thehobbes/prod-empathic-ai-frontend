...
function toCamelGraphEdge(edge) {
    ...
    return {
        ...
        sessionId: edge.sessionId ?? edge.session_id ?? edge.properties?.session_id ?? null,
...
function normalizeGraphSnapshotResponse(payload) {
    return {
        sessionId: payload?.sessionId ?? payload?.session_id ?? payload?.metadata?.session_id ?? null,
...
*/
export async function getSessionFullGraphSnapshot({ baseUrl, sessionId }) {
  const payload = await requestJson({
    baseUrl,
    path: `/v1/sessions/${sessionId}/graph/full`,
    method: "GET",
  });
